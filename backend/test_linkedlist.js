...
function addToMap(key, value) {
    ...
    if (coordinatesMap.has(key)) {
        // If it does, add to the existing array, ensure uniqueness, and sort it
        const existingArray = coordinatesMap.get(key);
        if (!existingArray.includes(value)) {
            existingArray.push(value);
            existingArray.sort((a, b) => a - b); // Sort the array
        }
    } else {
        // If not, create a new array with the value
        const newArray = [value];
        coordinatesMap.set(key, newArray);
    }
}
...
function constructLinkedListsAndMap(routeDetails) {
    routeDetails.forEach((route,index) => {
        ...
        routeSteps.forEach((step,ind) => {
            ...
            const stepHtmlInstructions = step.html_instructions;
            const stepElement={
                stepStartLocation,
                stepPolyline,
                stepDistance,
                stepDuration,
                stepHtmlInstructions
            };
            routeLegsArray.push(stepElement);

            // Add the start location to the map
            addToMap(JSON.stringify(stepStartLocation), {index,ind} );
            // console.log(routeLegsLinkedList);
        });
        let len=routeLegsArray.length;
        lastnode_location=routeLegs.steps[len-1].end_location;
        routeLegsArray.push({stepStartLocation: lastnode_location});
        addToMap(JSON.stringify(lastnode_location), {index,len} );
        endLocation=lastnode_location
        routeArrays.set(index, routeLegsArray);
    });
}

constructLinkedListsAndMap(routeDetails);

// console.log(routeArrays);
// console.log(coordinatesMap);

module.exports = {routeArrays, coordinatesMap,endLocation,routeDetails}